...
void construct_arg_desc(argdesc_vt& x) {
    ...
    x.push_back(new sarg_t("--trace-path", "./logs/", &Args::tracePath, "path to the log directory. relative path from repo root directory or absolute path possible"));
...
Args::Args() : _help(false), _trace(false), _measure(false), _print(false), _collectionPath("./data/"), _tracePath("./logs/"), _results(10), _tiers(0), _dimensions(0) {}
