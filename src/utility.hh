...
namespace IR {
    ...
    inline float calcTF(const std::string& term, const std::string& content) {
        ...
        return static_cast<float>((1 + log10(utility::string::countWordInString(content, term, false))) / (1 + log10(10)));
    }
    ...
    inline std::string stemPorter(const std::string& sentence) {
        ...
        while (iss >> word) {
            ...
            std::wstring wordToStem = utility::string::string2wstring(word);
            StemEnglish(wordToStem);
            os << utility::string::wstring2string(wordToStem) << " ";
        }

        std::string stemmed = os.str();
        utility::string::trim(stemmed);
...
namespace similarity_measures {
    ...
    inline float calcCosineSimilarityEfficient(){};
    ...
    /**
     * Returns 1 - <calcCosineSimilarity>"()". So we have the higher the returned value the more similar the docs are. This is just for negating the
     ...
     * @return
     */
    inline float calcCosineDistance(const Document& doc_a, const Document& doc_b) { return 1 - calcCosineSimilarity(doc_a, doc_b); }
