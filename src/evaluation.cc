...
IRPM::IR_PerformanceManager() : _queryScores(), _cb(nullptr) {}
...
void IRPM::init(const CB& aControlBlock) {
    if (!_cb) {
        ...
        while (std::getline(file, line)) {
            ...
            queryNames.insert(qID);
            _queryScores[qID].emplace_back(qID, dID, score);

            size_t size = _queryScores.at(qID).size() - 1;

            /*std::cout << "qID: " << qID << " : " << _queryScores.at(qID).at(size).getQueryID() << " : " << _queryScores.at(qID).at(size).getDocumentID()
                      << " : " << _queryScores.at(qID).at(size).getScore();*/
        }
        /*
        for(auto& elem: queryNames){
...
void IRPM::tp_tn_fp_fn(const std::string& aQueryID, const sizet_vt& aRanking, size_t& aTP, size_t& aTN, size_t& aFP, size_t& aFN) {
    std::cout << "in tp_tn_fp_fn" << std::endl;
    ...
    std::cout << "Utility::VecUtil::difference" << std::endl;
    sizet_vt lRelevant = getRelevantDocIDs(aQueryID); // ids of the relevant docs
    std::cout << "getRelevantDocIDs" << std::endl;
...
sizet_vt IRPM::getRelevantDocIDs(const std::string& aQueryID) {
    std::cout << "in getRelevantDocIDs" << std::endl;
    const auto& lQueryScores = getQueryScores(aQueryID);
    std::cout << "after getQueryScores" << std::endl;
    ...
    for (const auto& score : lQueryScores) {
        std::cout << "qid: " << score.getQueryID() << "doc id: " << score.getDocumentID() << std::endl;
        lRelevantDocIDs.push_back(DocumentManager::getInstance().getDocument(score.getDocumentID()).getID());
    }
    std::cout << "end getRelevantDocIDs" << std::endl;
...
Evaluation::Evaluation() :
    _irpm(IR_PerformanceManager::getInstance()), _evalResults(), _mode(""), _queryName(""), _evalPath(""), _measureInstance(nullptr), _time(0), _started(false),
    _cb(nullptr) {
...
void Evaluation::stop() {
    if (_started) {
        ...
        const std::string lTraceMsg =
            std::string("Time to process query '") + _queryName + std::string("' in mode '") + _mode + std::string("' : ") + std::to_string(_time);
        TRACE(lTraceMsg);
